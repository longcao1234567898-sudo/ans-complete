...
| P05 | 2026-09-23 | `TAI-LIEU` | Sắp xếp lại cấu trúc thư mục và nội dung tài liệu cho người đọc dễ định hướng; thay lịch 12 ngày đã chết bằng mốc theo phiên | **XONG** | `35673bd` | `5f61225` `c282449` `6ee203a` `0e79a91` `6d304a2` `57076b5` | Không còn phần nào của mục tiêu. **Cấu trúc:** `kiem-thu-bao-mat/` chia ba thư mục con theo loại nội dung — `phuong-phap/` dạy cách làm, `bieu-mau/` là form trống, `ket-qua/` là sản phẩm đã làm ra. Trước đây 13 tệp nằm phẳng, không phân biệt được. Đổi tên cho nhất quán, bỏ nhãn `PHU-LUC-A/B/C/D` vô nghĩa. Dùng `git mv` nên lịch sử từng tệp còn tra được. **Bản đồ mới:** `docs/README.md` định tuyến theo vai trò người đọc, nối vào `README.md` gốc và `CLAUDE.md`. **Hai thứ nói dối đã gỡ:** (1) bảng lịch 12 ngày D1–D12 ghi hôm nay phải đang retest và còn hai ngày là nộp báo cáo; (2) kế hoạch đặt trước số phiên cho từng việc, đã phải đánh số lại hai lần trong một ngày. Nay kế hoạch dùng mã giai đoạn `GĐ0-a`…`GĐ2-8` và ghi *làm gì*; tiến độ ghi *ai làm, khi nào*. **Kiểm:** 121 liên kết trong repo, 0 gãy, bằng script. **Một sai sót của chính phiên này, ghi lại để phiên sau tránh:** commit `5f61225` chỉ ghi được phần đổi tên tệp, mất phần sửa đường dẫn bên trong 10 tệp. Lệnh `git add` có kèm đường dẫn cũ đã không còn tồn tại nên git từ chối stage toàn bộ; lỗi bị nuốt bằng `2>/dev/null` và lệnh nối bằng `;` thay vì `&&` nên commit vẫn chạy. Đã vá ở `57076b5`. Bài học: kiểm `git diff --cached` trước khi commit, đừng tin là lệnh `add` đã chạy đúng. **Phiên sau:** GĐ0-a, tức `/phien-retest BUG-001`, **mở bằng cửa sổ Claude hoàn toàn mới**. |
| P06 | 2026-09-23 | `TAI-LIEU` | Ghi nhận BUG-005 phát hiện lúc quét an toàn trước push, và thêm cổng chặn đã để nó lọt qua năm phiên | **XONG** | `e9964f1` | `beecd3e` | Không còn phần nào của mục tiêu. Mở **BUG-005** (Critical) — chi tiết ở `buglogs/`, tài liệu công khai chỉ ghi mã. Thêm **mục 8 vào cổng DoD**, đồng bộ ở cả ba nơi: `/ket-thuc-phien`, QUY-TRINH §5, và sổ tay. Mục 8 khác bảy mục kia ở chỗ nó soát **toàn bộ cây tệp đang được theo dõi** chứ không soát diff — đó đúng là lý do bảy mục cũ không bắt được gì suốt năm phiên. ⚠️ **Cổng DoD mục 8 đang ĐỎ ở chính commit này, và đó là cố ý**: nó đang báo đúng ba tệp của BUG-005. Không vá trong phiên này vì việc xử lý cần quyết định của người vận hành (xoay mật khẩu, có viết lại lịch sử git hay không) và cần phối hợp với người cùng làm — kế hoạch đầy đủ theo thứ tự nằm trong BUG-005. **Phiên sau:** bước 1 của BUG-005 (xoay mật khẩu, thu hồi phiên), làm ngay và độc lập với lịch audit. Sau đó mới tới GĐ0-a. |
