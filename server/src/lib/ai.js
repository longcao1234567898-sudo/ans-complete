...
/** Đoán mức khẩn cấp từ nội dung — dùng khi KHÔNG gọi AI ngoài */
function guessUrgencyLocally(content) {
  const t = bo_dau(content);
  if (URGENT_KEYWORDS.some((kw) => t.includes(kw))) {
    return { suggestedUrgency: 'urgent', urgencyReason: 'Nội dung có dấu hiệu nguy hiểm, cần xử lý ngay.' };
  }
  if (IMPORTANT_KEYWORDS.some((kw) => t.includes(kw))) {
    return { suggestedUrgency: 'important', urgencyReason: 'Vụ việc nghiêm trọng, cần sớm quan tâm.' };
  }
...
5. suggestedUrgency: ĐÁNH GIÁ MỨC KHẨN CẤP dựa trên mức nguy hiểm và tính cấp bách:
   - "urgent" (KHẨN CẤP): có người đang bị nguy hiểm tính mạng/sức khoẻ; vụ việc
     ĐANG DIỄN RA cần lực lượng đến ngay; cháy nổ, đánh nhau, cướp giật, bắt cóc,
     bạo hành, đe doạ giết người, tai nạn nghiêm trọng, buôn bán ma tuý quy mô lớn.
   - "important" (QUAN TRỌNG): vụ việc nghiêm trọng nhưng KHÔNG đang diễn ra;
     trộm cắp đã xảy ra, lừa đảo chiếm đoạt tài sản, tụ tập gây rối, tệ nạn xã hội,
     vi phạm có nguy cơ tái diễn, khiếu nại kéo dài chưa được giải quyết.
   - "normal" (BÌNH THƯỜNG): phản ánh hạ tầng, môi trường, tiếng ồn, góp ý, đề xuất,
     hỏi thủ tục hành chính, các việc không có yếu tố nguy hiểm.

6. urgencyReason: MỘT CÂU NGẮN (dưới 25 từ) giải thích vì sao chọn mức đó,
   viết cho cán bộ đọc. Ví dụ: "Vụ việc đang diễn ra, có người bị thương".`;
    ...
    generationConfig: {
      ...
      responseSchema: {
        ...
        properties: {
          ...
          suggestedUrgency: { type: 'STRING', enum: ['normal', 'important', 'urgent'] },
          urgencyReason: { type: 'STRING' },
        },
        required: ['normalizedContent', 'suggestedCategory', 'confidence', 'keywords', 'suggestedUrgency'],
      },
  ...
  // Mức khẩn cấp AI đề xuất — người dân vẫn có quyền tự chọn lại
  const VALID_URGENCY = new Set(['normal', 'important', 'urgent']);
  if (!VALID_URGENCY.has(parsed.suggestedUrgency)) parsed.suggestedUrgency = 'normal';
  parsed.urgencyReason = String(parsed.urgencyReason || '').slice(0, 160);
