...
/** Đoán mức khẩn cấp từ từ khoá — dùng chung cho cả nhánh nội bộ và nhánh Gemini */
function doanMucKhanCap(khongDau) {
  if (URGENT_KEYWORDS.some((k) => khongDau.includes(k))) {
    return { suggestedUrgency: 'urgent', urgencyReason: 'Có dấu hiệu vụ việc đang diễn ra hoặc đe doạ an toàn tính mạng.' };
  }
  if (IMPORTANT_KEYWORDS.some((k) => khongDau.includes(k))) {
    return { suggestedUrgency: 'important', urgencyReason: 'Vụ việc có dấu hiệu nghiêm trọng nhưng không đang diễn ra.' };
  }
  return { suggestedUrgency: 'normal', urgencyReason: 'Không phát hiện dấu hiệu cần xử lý ngay.' };
}
...
/** Chuẩn hoá nhẹ: gộp khoảng trắng, viết hoa đầu câu, thêm dấu kết câu */
function dienDatLai(content, category) {
  const sach = String(content).trim().replace(/\s+/g, ' ');
  const hoa = sach.charAt(0).toUpperCase() + sach.slice(1);
  return `${CATEGORY_PREFIX[category]}: “${hoa}${/[.!?]$/.test(hoa) ? '' : '.'}”`;
}

/**
 * Phân tích HOÀN TOÀN NỘI BỘ — không một byte nào rời khỏi máy chủ.
 * Dùng cho nội dung tố giác, và cũng là phương án dự phòng khi Gemini lỗi.
 */
function phanTichNoiBo(content, laToGiac) {
  const khongDau = bo_dau(content);
  const tuKhoa = [...new Set(
    [...TO_GIAC_KEYWORDS, ...URGENT_KEYWORDS, ...IMPORTANT_KEYWORDS].filter((k) => khongDau.includes(k))
  )].slice(0, 5);

  return {
    aiSkipped: laToGiac,
    privacyNote: laToGiac
      ? 'Nội dung tố giác được phân tích ngay trên máy chủ của đơn vị, KHÔNG gửi sang bất kỳ dịch vụ AI bên ngoài nào.'
      : undefined,
    normalizedContent: dienDatLai(content, laToGiac ? 'to_giac' : 'phan_anh'),
    suggestedCategory: laToGiac ? 'to_giac' : 'phan_anh',
    confidence: tuKhoa.length === 0 ? 0.62 : Math.min(0.6 + tuKhoa.length * 0.12, 0.97),
    keywords: tuKhoa,
    ...doanMucKhanCap(khongDau),
  };
}

/**
 * Phân tích + phân loại một ý kiến.
 *
 * ⚠️ NỘI DUNG TỐ GIÁC KHÔNG BAO GIỜ RỜI MÁY CHỦ (xem khối giải thích ở trên):
 * quét từ khoá TRƯỚC, có dấu hiệu tố giác thì phân tích nội bộ và dừng tại đây.
 * Gemini lỗi cũng rơi về nhánh nội bộ — người dân không bao giờ bị chặn vì AI hỏng.
 */
export async function geminiAnalyze(content) {
  const khongDau = bo_dau(content);
  const laToGiac = TO_GIAC_KEYWORDS.some((k) => khongDau.includes(k));
  if (laToGiac) return phanTichNoiBo(content, true);

  try {
    const raw = await callGemini(GEMINI_MODEL, {
      systemInstruction: {
        parts: [{
          text: 'Bạn phân loại ý kiến người dân gửi cơ quan công an cấp xã. '
              + 'CHỈ trả về JSON thuần, không kèm giải thích, theo đúng dạng: '
              + '{"normalizedContent":"...","suggestedCategory":"to_giac|khieu_nai|phan_anh|de_xuat",'
              + '"confidence":0.0,"keywords":["..."]}. '
              + 'normalizedContent là nội dung diễn đạt lại rõ ràng, lịch sự, bằng tiếng Việt, giữ nguyên sự thật.',
        }],
      },
      ...
      generationConfig: {
        ...
        temperature: 0.2,
        ...thinking(GEMINI_MODEL),
      },
    });

    const kq = extractJson(raw);
    const cat = VALID_CATEGORIES.has(kq.suggestedCategory) ? kq.suggestedCategory : 'phan_anh';
    return {
      normalizedContent: String(kq.normalizedContent || '').trim() || dienDatLai(content, cat),
      suggestedCategory: cat,
      confidence: Math.min(Math.max(Number(kq.confidence) || 0.62, 0), 1),
      keywords: Array.isArray(kq.keywords) ? kq.keywords.map(String).slice(0, 5) : [],
      ...doanMucKhanCap(khongDau),
    };
  } catch (err) {
    console.warn('Gemini phân tích lỗi, dùng phân tích nội bộ:', err.message);
    return phanTichNoiBo(content, false);
  }
}

/**
 * Kiểm duyệt ảnh: true = nhạy cảm/khiêu dâm.
 *
 * Chỉ nhận data URL. Ảnh BẰNG CHỨNG TỐ GIÁC không đi qua đây — phía gọi đã
 * chặn từ trước (moderationService.ts), vì ảnh tố giác có thể chứa mặt người,
 * biển số, hiện trường.
 */
export async function geminiModerateImage(dataUrl) {
  const [header, base64] = String(dataUrl).split(',');
  if (!base64) throw new Error('Ảnh không đúng định dạng data URL');
  const mimeType = /data:(.*?);/.exec(header)?.[1] || 'image/jpeg';

  const text = await callGemini(GEMINI_MODEL, {
    contents: [{
      role: 'user',
      parts: [
        { inline_data: { mime_type: mimeType, data: base64 } },
        {
          text: 'Bạn là bộ lọc kiểm duyệt của cổng tiếp nhận ý kiến công dân. '
              + 'Ảnh trên có chứa nội dung khiêu dâm, khoả thân hoặc nhạy cảm tình dục không? '
              + 'Chỉ trả lời đúng MỘT từ: SENSITIVE hoặc SAFE.',
        },
      ],
    }],
    generationConfig: { maxOutputTokens: 512, temperature: 0, ...thinking(GEMINI_MODEL) },
  });
