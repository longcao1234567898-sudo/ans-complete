 ...
 */
function thinking(proBudget = 128) {
  return { thinkingConfig: { thinkingBudget: isPro() ? proBudget : 0 } };
}
...
export async function geminiChat(message, history = []) {
  ...
  return callGemini({
    ...
    generationConfig: {
      ...
      temperature: 0.4,
      ...thinking(256),   // Pro: suy nghĩ vừa đủ để trả lời nhanh
    },
  });
}
...
export async function geminiModerateImage(dataUrl) {
  ...
  const text = await callGemini({
    ...
    generationConfig: {
      maxOutputTokens: 20,
      temperature: 0,
      ...thinking(128),   // chỉ trả 1 từ, không cần suy nghĩ nhiều
    },
  });
