...
export async function sendOtpEmail(email, code) {
  ...
  try {
    ...
    // Gửi mail hỏng -> KHÔNG chặn người dân.
    // Quay về chế độ DEMO để họ vẫn gửi được ý kiến.
    console.error(`❌ Gửi email thất bại (${mode}):`, err.message);
    if (err.message.includes('timeout') || err.message.includes('ETIMEDOUT')) {
      console.error('   👉 Render thường CHẶN cổng SMTP. Hãy dùng RESEND_API_KEY (gửi qua HTTPS).');
    }
    return { sent: false, devCode: code, failed: true, reason: err.message };
  }
}
