...
export default function StepIndicator({ current }: { current: number }) {
  return (
    <div className="mb-8">
      <ol className="flex items-center" aria-label="Tiến trình gửi ý kiến">
        {STEPS.map((label, idx) => {
          const step = idx + 1;
      ...
      {/* TRÊN ĐIỆN THOẠI: nhãn từng bước bị ẩn (chỉ hiện từ cỡ sm trở lên) nên
          bà con chỉ thấy 5 vòng tròn số, không biết mình đang làm gì.
          Dòng này bù lại: nói rõ đang ở bước mấy trên mấy và bước đó tên gì. */}
      <p className="mt-3 text-center text-sm font-semibold text-primary-700 dark:text-primary-300 sm:hidden">
        Bước {current}/{STEPS.length} — {STEPS[current - 1]}
      </p>
    </div>
  );
}
