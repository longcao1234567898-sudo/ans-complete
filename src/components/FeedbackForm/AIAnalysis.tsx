  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-2">
        ...
        <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">{t('ai.heThongDangPhan')}</h3>
      </div>
          ...
          <>
            ...
            {result.topicLabel && (
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <span className="text-xs font-medium text-slate-500 dark:text-slate-400">{t('ai.chuDe')}</span>
            ...
            {result.keywords.length > 0 && (
              <div className="mt-4">
                <p className="mb-1.5 text-xs font-medium text-slate-500 dark:text-slate-400">{t('ai.tuKhoaNhanDien')}</p>
                <div className="flex flex-wrap gap-1.5">
      ...
      {!isLoading && result && (
        ...
        >
          <RefreshCw className="h-3.5 w-3.5" /> {t('ai.phanTichLai')}
        </button>
      )}
      ...
      {result && result.duRo === false && (
        <div className="mt-4 rounded-xl border-2 border-accent-300 bg-accent-50 p-4 dark:border-accent-700 dark:bg-accent-900/20">
          ...
          <p className="mb-2 text-sm text-slate-700 dark:text-slate-300">
            {t('ai.baConVuiLong')} <strong>{t('ai.quayLai')}</strong> {t('ai.vaBoSungGiup')}
          </p>
          ...
          <p className="mt-3 rounded-lg bg-white/70 px-3 py-2 text-xs text-slate-600 dark:bg-slate-800/60 dark:text-slate-400">
            {t('ai.viDu')} <em>{t('ai.toiQuaKhoang10')}</em>
          </p>
        </div>
      )}
